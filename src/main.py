...
def run_train(args):
    ...
    train_parse = [tree.convert() for tree in train_treebank] #check
    ...
    # with open('word_vocab.pkl','wb') as picklefile:
    #     pickle.dump(word_vocab,picklefile)
    ...
    for epoch in itertools.count(start=1):
        ...
        for start_index in range(0, len(train_parse), args.batch_size):
            ...
            batch_loss_value = batch_loss.data
            try:
                batch_loss.backward()
                # trainer.update()
                optimizer.step()
        ...
        torch.save(parser, "model1_epoch_"+str(epoch))
